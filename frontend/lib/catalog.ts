...
export const NODE_DEFS: NodeDef[] = [
  ...
  },
  {
    ...
    accent: "#f472b6",
    glyph: "⦿",
    description: "Run a VisionLab extraction task on an uploaded asset. Returns the structured result.",
    hasInput: true,
    outputs: [{ id: null }],
    fields: [
      { key: "task_id", label: "Task ID", type: "text", placeholder: "task_…", hint: "From vision.zoidlab.ai — a task's detail page." },
      { key: "asset_id", label: "Asset ID", type: "text", placeholder: "asset_…", hint: "An uploaded image's ID." },
      { key: "wait_s", label: "Max wait (s)", type: "number", default: 240, min: 30, max: 570 },
    ],
  },
  {
    type: "voice_run",
    ...
    accent: "#2dd4bf",
    glyph: "◍",
    description: "Simulate a VoiceLab agent against a caller scenario. Returns outcome + transcript.",
    hasInput: true,
    outputs: [{ id: null }],
    fields: [
      { key: "agent_id", label: "Agent ID", type: "text", placeholder: "agent_…", hint: "From voice.zoidlab.ai." },
      { key: "scenario_id", label: "Scenario ID", type: "text", placeholder: "scen_…" },
      { key: "max_turns", label: "Max turns", type: "number", default: 6, min: 2, max: 16 },
      { key: "wait_s", label: "Max wait (s)", type: "number", default: 240, min: 30, max: 570 },
    ],
  },
  {
    ...
    accent: "#818cf8",
    glyph: "⧉",
    description: "Call a tool on a governed MCPLab connector. Allow-lists and approval gates apply.",
    hasInput: true,
    outputs: [{ id: null }],
    fields: [
      { key: "connector_id", label: "Connector ID", type: "text", placeholder: "conn_…", hint: "From mcplab.zoidlab.ai." },
      { key: "tool", label: "Tool name", type: "text", placeholder: "search" },
      { key: "arguments", label: "Arguments (JSON)", type: "textarea", default: "{}", placeholder: '{"query": "{{previous.output}}"}' },
      { key: "wait_s", label: "Max wait (s)", type: "number", default: 240, min: 30, max: 570 },
    ],
  },
  {
    ...
    accent: "#fbbf24",
    glyph: "✺",
    description: "Hand a task to a SwarmLab multi-agent team. Returns the swarm's final output.",
    hasInput: true,
    outputs: [{ id: null }],
    fields: [
      { key: "swarm_id", label: "Swarm ID", type: "text", placeholder: "swarm_…", hint: "From swarm.zoidlab.ai." },
      { key: "task", label: "Task", type: "textarea", default: "{{previous.output}}", placeholder: "Empty = the incoming text." },
      { key: "wait_s", label: "Max wait (s)", type: "number", default: 240, min: 30, max: 570 },
    ],
  },
  {
